...
export default function HeroSection() {
  ...
  return (
    <section className="relative bg-surface-container-lowest rounded-[24px] border border-outline-variant shadow-sm overflow-hidden p-2xl min-h-[400px] flex items-center justify-center">
      ...
      <div className="relative z-10 flex w-full max-w-4xl flex-col items-center space-y-8 px-4 text-center">
        <div>
          {/* Nom de l'entreprise du compte connecté (voir useEntreprise.ts).
              N'apparaît que si une entreprise a déjà été créée. */}
          {entreprise && (
            <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-primary-container/30 px-4 py-1.5">
          ...
          <p className="w-full max-w-3xl font-body-lg text-body-lg text-on-surface-variant">
            {entreprise?.slogan ||
              "Prêt à transformer votre vision en réalité ? Notre intelligence artificielle est là pour vous guider à chaque étape du processus entrepreneurial."}
          </p>
        </div>

        {/* Le CTA change de sens selon qu'une entreprise existe déjà ou non.
            On attend la fin du chargement (loading) pour éviter d'afficher
            "Créer votre entreprise" une fraction de seconde avant de basculer
            sur "Configurer l'identité visuelle". */}
        {!loading && !entreprise && (
        ...
        {entreprise && (
          <Button
            href={`/PaletteColor?entrepriseId=${entreprise.id}`}
            size="lg"
            className="text-base"
          >
